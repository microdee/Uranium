...
private:
	...
	void SubscribeToEvents(TWeakObjectPtr<UUrBrowserView> browser);
	void RemoveFromEvents(TWeakObjectPtr<UUrBrowserView> browser);
	
	void HandleOnPopupShow(UUrBrowserView* browser, bool show);
	void HandleOnPopupSize(UUrBrowserView* browser, FVector2D location, FVector2D size);
