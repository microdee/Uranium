...
CefRefPtr<CefV8Value> CefV8Utils::to_v8array(CefRefPtr<CefListValue> const& list, int reclevel /*= 0 */)
{
	if (reclevel >= CEFV8_UTILS_MAX_RECLEVEL)
	{
		...
		errorArr->SetValue(0, CefV8Value::CreateString("max recursion level reached"));
		return errorArr;
	}
	if(list)
	{
		...
		for(int i=0; i<arr->GetArrayLength(); i++)
		{
			...
			switch (type)
			{
			case VTYPE_BOOL: arr->SetValue(i, CefV8Value::CreateBool(list->GetBool(i)));
				break;
			case VTYPE_INT: arr->SetValue(i, CefV8Value::CreateInt(list->GetInt(i)));
				break;
			case VTYPE_DOUBLE: arr->SetValue(i, CefV8Value::CreateDouble(list->GetDouble(i)));
				break;
			case VTYPE_STRING: arr->SetValue(i, CefV8Value::CreateString(list->GetString(i)));
				break;
			case VTYPE_DICTIONARY: arr->SetValue(i, to_v8object(list->GetDictionary(i), reclevel + 1));
				break;
			case VTYPE_LIST: arr->SetValue(i, to_v8array(list->GetList(i), reclevel + 1));
			default:
				break;
			}
		}
...
CefRefPtr<CefListValue> CefV8Utils::to_list(CefRefPtr<CefV8Value> const& arr, int reclevel /*= 0 */)
{
	...
	else if(arr->IsArray())
	{
		for(int i=0; i<arr->GetArrayLength(); i++)
		{
			CefRefPtr<CefV8Value> v = arr->GetValue(i);
